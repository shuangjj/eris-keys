...
do
	...
	NAME=testkey1
	ADDR=`eris-keys gen --type $KEYTYPE --name $NAME`
	echo $ADDR
	echo $NAME
	...
	VERIFY1=`eris-keys verify --name $NAME $HASH $SIG1`
	if [ $VERIFY1 != "true" ]; then
		echo "FAILED verify: got $VERIFY1 expected true"
		exit 1
	fi
	VERIFY2=`eris-keys verify --addr $ADDR $HASH $SIG1`
	if [ $VERIFY2 != "true" ]; then
		echo "FAILED verify: got $VERIFY2 expected true"
		exit 1
	fi

	SIG2=`eris-keys sign --addr $ADDR $HASH`
	VERIFY1=`eris-keys verify --name $NAME $HASH $SIG2`
	if [ $VERIFY1 != "true" ]; then
		echo "FAILED verify: got $VERIFY1 expected true"
		exit 1
	fi
	VERIFY2=`eris-keys verify --addr $ADDR $HASH $SIG2`
	if [ $VERIFY2 != "true" ]; then
		echo "FAILED verify: got $VERIFY2 expected true"
		exit 1
	fi
...
do
	...
	ADDR=`eris-keys gen --type $KEYTYPE`
